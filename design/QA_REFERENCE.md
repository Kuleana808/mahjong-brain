...
- Tile art uses heavy original silhouettes across roughly 89% of the usable face width. Dots use saturated red/cobalt/emerald layered rings, bamboo has broad emerald segmented stalks, and characters/winds/dragons use saturated ink with the heaviest available CJK weight. Thin, muddy diagram marks and tiny metadata fail readability QA.
...
| `S02-age-rest` | Age range | Three ivory range choices, no preselection, and no choice blocks play. |
...
| `S07-home-new` | New home | Brand hierarchy, Level 1 primary CTA, then Account, Appearance, and Settings medallions at the approved anchors. Unconfigured native services stay truthful inside their surfaces. |
...
| Gameplay chrome | `top-back`, `top-count`, `top-menu`, `bottom-shuffle`, `bottom-hint`, `bottom-undo`, `carved-walnut-controls` |
