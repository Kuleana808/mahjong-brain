...
| `app-icon` | Brand tile in emerald frame | 1024 px master plus iOS rendition | SVG master + PNG asset catalog | approved | No transparency; no text; survives 29 px; emerald/ivory/bronze only. Source: `design/assets/app-icon-master.svg`; release fingerprint: `design/brand-assets.lock.json`. |
| `splash` | Centered wordmark and brand tile | iPhone/iPad, lightless emerald | SVG master + PNG asset catalog | approved | No white flash; no progress UI; safe at all aspect ratios. Source: `design/assets/splash-master.svg`; release fingerprint: `design/brand-assets.lock.json`. |
