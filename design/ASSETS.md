...
|---|---|---|---|---|---|
| `brand-mark` | Energetic match-burst brain tile | full and compact | Generated PNG rendered in app UI | approved | Same master composition as the App Store icon for one recognizable identity; mirrored emerald lobes, mint pips, and amber seam remain legible at 60 px. Source: `design/assets/app-icon-generated-source.png`, web rendition: `public/brand-mark.png`. |
...
| `app-icon` | Energetic brain tile in emerald frame | generated source plus 1024 px iOS rendition | Opaque PNG source + asset catalog | approved | No transparency, words, or language dependency; the mirrored brain silhouette survives 60 px; emerald/ivory/mint/amber/bronze. Source: `design/assets/app-icon-generated-source.png`; release fingerprint: `design/brand-assets.lock.json`. |
| `splash` | Centered energetic brain-tile brand mark | iPhone/iPad, lightless emerald | Deterministic PNG asset catalog | approved | Same mark as the icon/logo; no white flash or progress UI; safe at all aspect ratios. Generated by `scripts/render-brand-splash.py`; release fingerprint: `design/brand-assets.lock.json`. |
...
| `tile-faces` | Full mahjong set | Modern, Classic, Porcelain, and Brain treatments across all suits, winds, dragons, flowers, and seasons | Canvas code | approved code | Bold central motif; traditional characters in Classic; outlined geometric marks in Porcelain; small branded brain signature in Brain; suit shape distinct without color; no external art. |
...
| `game-sounds` | Original interaction audio | tile, blocked, match, holder warning/full, hint, shuffle, undo, win, and low-volume ambient music | Web Audio synthesis | approved code | Quiet ceramic/wood cues plus a separate original three-voice ambient bed; no borrowed samples or casino fanfare; Music and Sounds toggles are independent. |
...
| `app-store-screens` | Store listing screenshots | required iPhone/iPad sizes | PNG/JPEG | approved source composition | Generated from deterministic QA fixtures, carrying the brain-tile brand signature and only claims present in the build. Final release-candidate recapture remains a release gate. |
