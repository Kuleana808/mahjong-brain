/**
 * Apple Root CA - G3, DER encoded and pinned at build time.
 *
 * Source: https://www.apple.com/certificateauthority/AppleRootCA-G3.cer
 * SHA-256: 63:34:3A:BF:B8:9A:6A:03:EB:B5:7E:9B:3F:5F:A7:BE:
 *             7C:4F:5C:75:6F:30:17:B3:A8:C4:88:C3:65:3E:91:79
 *
 * This is public trust material, not a secret. It must never be fetched at
 * runtime: the point of a trust anchor is that an attacker cannot replace it.
 */
export const APPLE_ROOT_CA_G3_BASE64 =
  '';
